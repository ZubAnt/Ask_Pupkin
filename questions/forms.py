...
from questions.models import Profile, Questions, Tags, Answers
...
class RegisterForm(forms.Form):
    ...
    def clean(self):
        ...
        if data['password'] != data['repeat_password']:
            ...
            self.add_error('password', forms.ValidationError(msg))
...
class AnswerForm(forms.Form):
    ...
    def save(self, user, question):
        data = self.cleaned_data
        profile = Profile.objects.get(user=user)
        ans = Answers.objects.create(profile=profile, question=question, text=data['answer'])
        ans.save()
